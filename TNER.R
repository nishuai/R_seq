...
#################################################################
# args=c('training/18B99984XJ_1.umi.pileup.txt', 'training/trained_BMER.rds',  0.01)
...
polish.pred.3nt=function(shrinkage_rate,  #basian posterior mutation rate and depth information
  ...
  CI_limit=CI_limit[I,]
  input_pileup=cbind(input_pileup, shrinkage_rate[I,])
  input_pileup=cbind(input_pileup, bg_rate[I,4:7])
  colnames(input_pileup)[10:17]=c(paste0('upperCI_', c('A','C','G','T')),
  ...
  ##the trained cutoff value is the lower CI at 0.99999 confidence for haploid SNP in training patients
  pred=as.logical(rowSums(input_rate_adj>CI_limit) & apply(input_rate,1,max)<0.271)
