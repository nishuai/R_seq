...
source('find_cutoff.R')
cat('Start computing')
args = commandArgs(trailingOnly=TRUE)
 # args=c('training/','ref/ref_seqs_panel.txt','ref/Target_v2.call.bed','.')
if (length(args)<1) {
...
for (filename in training_files){
  ...
  row.names(bb)=bb$id
  bb=bb[,c('chr.x','position','ref_base.x','A','C','G','T')]
  colnames(bb)=c('CHR','POSITION','REF','A','C','G','T')
  bb$REF=toupper(bb$REF)
...
##The real base mutation frequency without removing mutation rate in SNPs
...
##find real mutation after cutoff value calibration for each individual input dataset
##this is to more accurately calculate the 3-mer mutation rate
cutoff_nt_rate=function(All_training_list){
  nt_matrix=list()
  for (i in 1:length(All_training_list)){
    aa=All_training_list[[i]]
    aa[,4:7]=aa[,4:7]/rowSums(aa[,4:7])
    aa[,4:7][is.na(aa[,4:7])]=0
    aa[aa$REF=='A','A']=0
    aa[aa$REF=='C','C']=0
    aa[aa$REF=='G','G']=0
    aa[aa$REF=='T','T']=0
    cutoff=find_cutoff(aa[,4:7])
    aa[,4:7][aa[,4:7]>cutoff]=0
    nt_matrix[[i]]=aa[,4:7]
  }
  
  All_nt_rate=Reduce('+', nt_matrix)/i
  hs_bg_ave=cbind(All_training_list[[1]][,1:3],All_nt_rate)
  hs_bg_ave$REF=toupper(hs_bg_ave$REF)
  hs_bg_ave=hs_bg_ave[order(rownames(hs_bg_ave)),]
  return(hs_bg_ave)
}
...
cutoff_rate=cutoff_nt_rate(All_training_list = All_training)
...
## assumed pattern of distribution, therefore they are excluded from the global 3-mer
